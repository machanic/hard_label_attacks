...
def read_json_data(json_path):
    ...
    with open(json_path, "r") as file_obj:
        ...
        elif "surrogate_defense_model" in data_json["args"] and data_json["args"]["surrogate_defense_model"]:
            surrogate_defense_models.append(data_json["args"]["surrogate_defense_model"])
            if "surrogate_defense_norm" in data_json["args"]:
                surrogate_defense_norms.append(data_json["args"]["surrogate_defense_norm"])
            if "surrogate_defense_eps" in data_json["args"]:
                surrogate_defense_eps.append(data_json["args"]["surrogate_defense_eps"])
...
method_name_to_paper = {"tangent_attack":"TA",
                        ...
                        "HSJA":"HSJA", "SignOPT":"Sign-OPT", "SVMOPT":"SVM-OPT",
                         "Evolutionary":"Evolutionary", "SurFree":"SurFree","BBA":"BBA","SQBA":"SQBA",
...
def draw_query_distortion_figure(dataset, norm, targeted, arch, fig_type, dump_file_path, xlabel, ylabel,
    ...
    markers = [
        ...
        'x',  # x
        'd',  # thin diamond
        'H',  # hexagon2 (alternative hexagon)
        'p',  # pentagon
        "1",
        "2",
        "3",
    ...
    max_y = 0
    min_y= 999
    label_mapping = {"BBA$_{\mathrm{AT(ResNet50,\epsilon_{\ell_2}=3)}}$": r"BBA$_{\mathrm{3rd\ AT}}$",
                     "BBA$_{\mathrm{AT(ResNet50,\epsilon_{\ell_\infty}=\\frac{{4}}{{255}})}}$": r"BBA$_{\mathrm{2nd\ AT}}$",
                     "BBA$_{\mathrm{AT(ResNet50,\epsilon_{\ell_\infty}=\\frac{{8}}{{255}})}}$": r"BBA$_{\mathrm{1st\ AT}}$",
                     "SQBA$_{\mathrm{AT(ResNet50,\epsilon_{\ell_2}=3)}}$": r"SQBA$_{\mathrm{3rd\ AT}}$",
                     "SQBA$_{\mathrm{AT(ResNet50,\epsilon_{\ell_\infty}=\\frac{{4}}{{255}})}}$": r"SQBA$_{\mathrm{2nd\ AT}}$",
                     "SQBA$_{\mathrm{AT(ResNet50,\epsilon_{\ell_\infty}=\\frac{{8}}{{255}})}}$": r"SQBA$_{\mathrm{1st\ AT}}$",
                     "Prior-Sign-OPT$_{\mathrm{AT(ResNet50,\epsilon_{\ell_\infty}=\\frac{{8}}{{255}})}}$": r"Prior-Sign-OPT$_{\mathrm{1st\ AT}}$",
                     "Prior-Sign-OPT$_{\mathrm{AT(ResNet50,\epsilon_{\ell_\infty}=\\frac{{4}}{{255}})}}$": r"Prior-Sign-OPT$_{\mathrm{2nd\ AT}}$",
                     "Prior-Sign-OPT$_{\mathrm{AT(ResNet50,\epsilon_{\ell_\infty}=\\frac{{8}}{{255}})&AT(ResNet50,\epsilon_{\ell_2}=3)}}$": r"Prior-Sign-OPT$_{\mathrm{double\ ATs}}$",
                     "Prior-Sign-OPT$_{\mathrm{AT(ResNet50,\epsilon_{\ell_\infty}=\\frac{{4}}{{255}})&AT(ResNet50,\epsilon_{\ell_2}=3)}}$": r"Prior-Sign-OPT$_{\mathrm{double\ ATs}}$",
                     "Prior-OPT$_{\mathrm{AT(ResNet50,\epsilon_{\ell_\infty}=\\frac{{8}}{{255}})}}$":  r"Prior-OPT$_{\mathrm{1st\ AT}}$",
                     "Prior-OPT$_{\mathrm{AT(ResNet50,\epsilon_{\ell_\infty}=\\frac{{4}}{{255}})}}$": r"Prior-OPT$_{\mathrm{2nd\ AT}}$",
                     "Prior-OPT$_{\mathrm{AT(ResNet50,\epsilon_{\ell_\infty}=\\frac{{8}}{{255}})&AT(ResNet50,\epsilon_{\ell_2}=3)}}$": r"Prior-OPT$_{\mathrm{double\ ATs}}$",
                     "Prior-OPT$_{\mathrm{AT(ResNet50,\epsilon_{\ell_\infty}=\\frac{{4}}{{255}})&AT(ResNet50,\epsilon_{\ell_2}=3)}}$": r"Prior-OPT$_{\mathrm{double\ ATs}}$",
                     }
    all_used_colors = set()
    ...
    for idx, ((dataset, norm, targeted, method, surrogate_archs), (x,y)) in enumerate(data_info.items()):
        ...
        plt.plot(x, y, label=label_mapping.get(method, method), color=method_linestyle_mark_dict[method][2],
                         linestyle=method_linestyle_mark_dict[method][0], linewidth=2,
                         marker=method_linestyle_mark_dict[method][1], markersize=8, alpha=0.7)
    ...
    if "jpeg" in dump_file_path:
        plt.legend(loc='upper right', prop={'size': 18}, handlelength=3, handletextpad=0.5, borderpad=0.2,
                   framealpha=0.6, fancybox=True, frameon=True, ncol=2, columnspacing=0.3)
    elif "ImageNet" == dataset:
        plt.legend(loc='upper right',bbox_to_anchor=(1, 1.01), prop={'size': 18},
                   handlelength=3, handletextpad=0.5, borderpad=0.2,
                   framealpha=0.6, fancybox=True, frameon=True, ncol=2, columnspacing=0.3)
    elif ("com_defend" in dump_file_path or "feature_distillation" in dump_file_path) and "CIFAR-10" == dataset and targeted:
        plt.legend(loc='upper right', prop={'size': 18}, handlelength=3, handletextpad=0.5, borderpad=0.2,
               framealpha=0.6, fancybox=True, frameon=True, ncol=2, columnspacing=0.3)
    else:
        plt.legend(loc='upper right',bbox_to_anchor=(1.01, 1.01), prop={'size': 18},
                   handlelength=3, handletextpad=0.5, borderpad=0.2,
                   framealpha=0.6, fancybox=True, frameon=True, ncol=2, columnspacing=0.3)
...
# def draw_unified_legend(method_linestyle_mark_dict, dump_file_path):
#     plt.figure(figsize=(50, 2.5))
#     for (linestyle, mark, color, paper_legend_method) in method_linestyle_mark_dict.values():
#         plt.plot([], [], label=paper_legend_method, color=color,
#                  linestyle=linestyle, linewidth=2,
#                  marker=mark, markersize=8)
#     plt.legend(loc='center', fontsize=12, ncol=5)  # 设置每排放置8条线
#     plt.axis('off')
#     plt.savefig(dump_file_path, dpi=200)
#     print("save legend to {}".format(dump_file_path))
...
if __name__ == "__main__":
    args = parse_args()
    dump_folder = "D:/黑盒攻击论文/hard-label attacks/Prior-OPT/ICLR 2025/figures/query_vs_distortion/attack_defense/"
    os.makedirs(dump_folder, exist_ok=True)
    norm = "l2"
    all_used_colors = set()

    legend_file_path = dump_folder + "legend.pdf"
    for dataset in ["ImageNet","CIFAR-10"]:
        ...
        for targeted in [False]:
            if "CIFAR" in dataset:
                ...
                archs= ["resnet50(AT_linf_4_div_255)", "resnet50(AT_linf_8_div_255)"]
            for model in archs:
                ...
                draw_query_distortion_figure(dataset, args.norm, targeted, model, args.fig_type,
                                             file_path, x_label, y_label)
