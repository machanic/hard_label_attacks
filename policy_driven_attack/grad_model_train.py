...
from policy_driven_attack.pda_models.victim import VictimQuery
from policy_driven_attack.pda_models import make_policy_model
...
def parse_args():
    ...
    parser.add_argument('--model-dir', default='{}/train_pytorch_model/policy_driven_attack/pretrained_models'.format(PROJECT_PATH),type=str,
                        help='directory to save models')
    parser.add_argument('--npz-dir', default='{}/train_pytorch_model/policy_driven_attack/pretrained_data'.format(PROJECT_PATH), type=str,
                        help='direction where .npz data is stored')
    parser.add_argument('--training-data-victim-arch', type=str, required=True,
                        help='which victim-arch to use for pre-training')
    ...
    parser.add_argument('--dataset', default='mnist', type=str, choices=['mnist01', 'mnist', 'CIFAR-10', 'ImageNet'],
    ...
    with torch.no_grad():
        for phase in train_phases:
            ...
            for batch_index, (adv_image, image, label, grad) in enumerate(grad_loader[phase]):
                ...
                target = argsort[:, 0] * (~argsort[:, 0].eq(label)).long() + \
...
def test(model_file_dir, train_image_ids, stage1_image_ids, loader, model, policy, gpu, num_gpu_per_node, args):
    ...
    with torch.no_grad():
        for batch_index, (image_id, image, label) in enumerate(loader):
            ...
            with torch.enable_grad():
                ...
                loss = logit[torch.arange(image.shape[0]).long(), target.long()] - logit[torch.arange(image.shape[0]).long(), label.long()]
...
def train(optimizer, epoch_index, train_loader, model, policy, gpu, args):
    ...
    for batch_index, (adv_image, image, label, grad) in enumerate(train_loader):
        ...
        if 'grad' in output:
            ...
            assert grad.shape == pred.shape
            sim = (grad * pred).sum(dim=(1, 2, 3)) / norm(grad).view(-1) / norm(pred).view(-1)
            sim_all[selected] = sim.detach().cpu()#--[debug]
        ...
        if args.policy_arch.endswith('_inv'):
            ...
            ce_all[selected] = ce.detach().cpu()#--[debug]
            ...
            assert not is_warmup
            loss = -sim
        loss_all[selected] = loss.detach().cpu()#--[debug]
        ...
        # clip grad norm
        grad_all[selected] = torch.nn.utils.clip_grad_norm_(functools.reduce(lambda g1, g2: g1['params'] + g2['params'], optimizer.param_groups),args.clip_grad, norm_type=2).cpu()
        for param_group in optimizer.param_groups:
            for p in param_group['params']:
                if p.grad is not None:
                    clip_grad_all[selected] += p.grad.cpu().detach().norm(2) ** 2
...
def main_worker(gpu, num_gpu_per_node, args):
    ...
    # make victim model
    net = StandardModel(args.dataset, args.victim_arch, no_grad=False).eval()
    model = VictimQuery(net)
    ...
    with open(osp.join(args.npz_dir, args.dataset, args.training_data_victim_arch, 'image_ids_step.json')) as f:
        ids = json.load(f)
    stage1_image_ids = []
    for all_key in ids.keys():
        stage1_image_ids.append(int(all_key))
    ...
    model_file_dir = osp.join(args.model_dir, args.dataset, '{}_{}'.format(args.policy_arch, args.training_data_victim_arch))
    if (not args.use_ddp) or (args.use_ddp and args.ddp_rank % num_gpu_per_node == 0):
        fname = osp.join(model_file_dir, 'stage1_image_ids.pth')
        os.makedirs(osp.dirname(fname), exist_ok=True)
    ...
    for phase in ['train_seen', 'train_unseen']:
        ...
        if (not args.use_ddp) or (args.use_ddp and args.ddp_rank % num_gpu_per_node == 0):
            fname = osp.join(model_file_dir, '{}_image_ids.pth'.format(phase))
            os.makedirs(osp.dirname(fname), exist_ok=True)
    ...
    if not args.no_init_eval:
        ...
        gc.collect()
        torch.cuda.empty_cache()
        test(model_file_dir, train_image_ids, stage1_image_ids, loader, model, policy, gpu, num_gpu_per_node, args)
    ...
    for epoch_index in range(args.epochs):
        ...
        if result.get('train_unseen_tuple_sim', -1) > best_train_unseen_tuple_sim:
            ...
            save_checkpoint(policy, osp.join(model_file_dir, 'model_best.pth'),
        ...
        if args.save_every_epoch or (epoch_index + 1 in args.step_at) or epoch_index == args.warmup_epochs - 1:
            save_checkpoint(policy, osp.join(model_file_dir, 'model_epoch_{}.pth'.format(epoch_index)),
                            args.use_ddp, args.ddp_rank, num_gpu_per_node)
    # save model
    save_checkpoint(policy, osp.join(model_file_dir, 'model_final.pth'),
                    args.use_ddp, args.ddp_rank, num_gpu_per_node)
...
class GradDataset(torch.utils.data.Dataset):
    def __init__(self, args, tuple_fnames, use_true_grad, input_size,pre_load=True):
        ...
        json_and_npy_dir = self.args.npz_dir
        self.grad_key = 'true_grad' if use_true_grad else 'grad'
        with open(osp.join(json_and_npy_dir, args.dataset, args.training_data_victim_arch, 'image_ids_step.json')) as f:
            ids = json.load(f)
        self.id_index = []
        for id in ids.keys():
        ...
        adv_data = np.memmap(osp.join(json_and_npy_dir, args.dataset, args.training_data_victim_arch, 'adv_iamge.npy'), dtype=np.float32, mode='r', shape=(self.channel, self.size, self.size),
                             offset=0).copy()
        adv_data = adv_data.reshape(1, self.channel, self.size, self.size)
        torch_adv_data = torch.from_numpy(adv_data)
        if self.grad_key == 'true_grad':
            grad_data = np.memmap(osp.join(json_and_npy_dir, args.dataset, args.training_data_victim_arch, 'true_grad.npy'), dtype=np.float32, mode='r', shape=(self.channel, self.size, self.size),offset=0).copy()
            grad_data = grad_data.reshape(1, self.channel, self.size, self.size)
            torch_grad_data = torch.from_numpy(grad_data)
        else:
            grad_data = np.memmap(osp.join(json_and_npy_dir, args.dataset, args.training_data_victim_arch, 'grad_save.npy'), dtype=np.float32, mode='r', shape=(self.channel, self.size, self.size), offset=0).copy()
            grad_data = grad_data.reshape(1, self.channel, self.size, self.size)
            torch_grad_data = torch.from_numpy(grad_data)
        # get image ids
        self.image_ids = [int(id) for id in self.id_index]
        self.ids_step = [ids[id] for id in ids.keys()]
        self.image_shape = torch_adv_data.shape
        assert len(self.image_shape) == 4 and self.image_shape[0] == 1
        self.image_shape = self.image_shape[1:]
        self.grad_shape = torch_grad_data.shape
        ...
        if self.pre_load:
            ...
            for index in range(len(self.id_index)):
    ...
    def load_from_disk(self, index, no_pkl=False):
        ...
        if not no_pkl:
            with open(osp.join(json_and_npy_dir, self.args.dataset, self.args.training_data_victim_arch, 'images_and_labels.json')) as f:
                ial = json.load(f)

            label = torch.LongTensor([ial[str(image_id)]])[0]
            offset_index = self.find_key_index(osp.join(json_and_npy_dir, self.args.dataset, self.args.training_data_victim_arch),str(image_id))
            offset_index = int(offset_index)
            image_data = np.memmap(osp.join(json_and_npy_dir, self.args.dataset, self.args.training_data_victim_arch, 'image.npy'), dtype=np.float32, mode='r', shape=(self.channel, self.size, self.size), offset=offset_index * self.channel * self.size * self.size * 32 // 8).copy()
            image_data = image_data.reshape(1, self.channel, self.size, self.size)
        ...
        # load tuple
        new_adv_data = np.memmap(osp.join(json_and_npy_dir, self.args.dataset, self.args.training_data_victim_arch, 'adv_iamge.npy'), dtype=np.float32, mode='r', shape=(self.channel, self.size, self.size), offset=index * self.channel * self.size * self.size * 32 // 8).copy()
        new_adv_data = new_adv_data.reshape(1, self.channel, self.size, self.size)
        new_adv_data = torch.from_numpy(new_adv_data)
        adv_image = new_adv_data.view(self.image_shape)
        del new_adv_data
        if self.grad_key == 'true_grad':
            grad_data = np.memmap(osp.join(json_and_npy_dir, self.args.dataset, self.args.training_data_victim_arch, 'true_grad.npy'), dtype=np.float32, mode='r', shape=(self.channel, self.size, self.size), offset=index * self.channel * self.size * self.size * 32 // 8).copy()
            grad_data = grad_data.reshape(1, self.channel, self.size, self.size)
            torch_grad_data = torch.from_numpy(grad_data)
        else:
            grad_data = np.memmap(osp.join(json_and_npy_dir, self.args.dataset, self.args.training_data_victim_arch, 'grad_save.npy'), dtype=np.float32, mode='r', shape=(self.channel, self.size, self.size), offset=index * self.channel * self.size * self.size * 32 // 8).copy()
            grad_data = grad_data.reshape(1, self.channel, self.size, self.size)
            torch_grad_data = torch.from_numpy(grad_data)
        grad = torch_grad_data.view(self.grad_shape)
        del grad_data
        # return results
        return adv_image, image, label, grad

    def find_key_index(self, json_fname, key):
        index_json = osp.join(json_fname, 'image_ids_index.json')
...
if __name__ == '__main__':
    xargs = parse_args()
    os.environ['CUDA_VISIBLE_DEVICES'] = str(xargs.gpu)
