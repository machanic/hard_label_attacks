...
import policy_driven_attack.pda_models.standard_model
from policy_driven_attack.pda_models import policy
import policy_driven_attack.pda_models.policy
...
def make_policy_model(dataset, arch, input_size=0, **kwargs):
    assert dataset in ['debug', 'mnist01', 'mnist', 'CIFAR-10', 'ImageNet']#--[change]
    ...
    if dataset == 'debug':
        if arch == 'empty':
            model = policy_driven_attack.pda_models.policy.debug.empty(**kwargs)
        else:
    ...
    return policy_driven_attack.pda_models.standard_model.StandardPolicyModel(net=model)
