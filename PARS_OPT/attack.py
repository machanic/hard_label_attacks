...
def get_parse_args():
    ...
    surrogate_arch_group = parser.add_mutually_exclusive_group()
    ...
    parser.add_argument('--dim-reduce-factor', type=int, default=4)
...
if __name__ == "__main__":
    ...
    args.exp_dir = osp.join(args.exp_dir,
                            get_exp_dir_name(args.dataset, args.norm, args.targeted, args.target_type, args.method))  # 随机产生一个目录用于实验
    ...
    if args.all_archs:
        ...
        if args.attack_defense:
            if args.defense_model == "adv_train_on_ImageNet":
                log_file_path = osp.join(args.exp_dir, "run_defense_{}_{}_{}_{}.log".format(
                    args.arch, args.defense_model, args.defense_norm, args.defense_eps))
            else:
    ...
    if args.surrogate_archs is not None:
        if args.attack_defense:
            if args.defense_model == "adv_train_on_ImageNet":
                log_file_path = osp.join(args.exp_dir, "run_{}_surrogates_{}_defense_{}_{}_{}.log".format(
                    args.arch, ",".join(args.surrogate_archs), args.defense_model, args.defense_norm, args.defense_eps))
            else:
                log_file_path = osp.join(args.exp_dir, 'run_{}_surrogates_{}_defense_{}.log'.format(
                    args.arch,  ",".join(args.surrogate_archs), args.defense_model))
        else:
            log_file_path = osp.join(args.exp_dir, 'run_{}_surrogates_{}.log'.format(args.arch,  ",".join(args.surrogate_archs)))
    if args.ablation_study:
        log_file_path = osp.join(args.exp_dir, 'run_{}_dim_reduce_factor_{}.log'.format(args.arch, args.dim_reduce_factor))
    ...
    elif args.surrogate_archs is not None:
    ...
    for arch in archs:
        if args.attack_defense:
            if args.defense_model == "adv_train_on_ImageNet":
                save_result_path = args.exp_dir + "/{}_{}_{}_{}_result.json".format(
                    arch, args.defense_model, args.defense_norm, args.defense_eps)
            else:
        ...
        if args.surrogate_archs is not None:
            if args.attack_defense:
                if args.defense_model == "adv_train_on_ImageNet":
                    save_result_path = args.exp_dir + "/{}_surrogates_{}_{}_{}_{}_result.json".format(
                        arch,  ",".join(args.surrogate_archs), args.defense_model, args.defense_norm, args.defense_eps)
                else:
                    save_result_path = args.exp_dir + "/{}_surrogates_{}_{}_result.json".format(
                        arch, ",".join(args.surrogate_archs), args.defense_model)
            else:
                save_result_path = args.exp_dir + "/{}_surrogates_{}_result.json".format(arch, ",".join(args.surrogate_archs))

        if args.ablation_study:
            save_result_path = args.exp_dir + "/{}_with_{}_dim_reduce_factor_result.json".format(arch, args.dim_reduce_factor)
        ...
        for surrogate_model in surrogate_models:
            assert model.input_size[-1] == surrogate_model.input_size[-1], \
                "Error, the input sizes (image's dimension) of the target model and the surrogate model do not equal!"
        tol = None
        ...
        if args.norm == "l2":
            attacker = ParsOptL2Norm(model, surrogate_models, args.dataset, args.epsilon, args.targeted,
                                     ...
                                     resized=args.resized, dim_reduce_factor=args.dim_reduce_factor,
        ...
        #                                args.batch_size, args.est_grad_samples, alpha=args.lr, maximum_queries=args.max_queries,
