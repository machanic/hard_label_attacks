...
class SignOptLinfNorm(object):
    def __init__(self, model, dataset, epsilon, targeted, batch_size=1, k=100, alpha=0.2, beta=0.001, iterations=1000,
    ...
    def sign_grad(self, images, true_label, theta, initial_lbd, h=0.001, target_label=None):
        ...
        sign(g) = 1/Q [ \sum_{q=1}^Q sign( g(theta+h*u_i) - g(theta) )u_i$ ]
        """
        K = self.k  # 100 random directions (for estimating the gradient)
        initial_lbd = self.convert_linf_to_l2_distance(theta, initial_lbd)  # 很奇怪，为何要用相同的半径
