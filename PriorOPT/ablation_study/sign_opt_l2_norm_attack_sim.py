...
class SignOptL2Norm(object):
    def __init__(self, model, dataset, epsilon, targeted, batch_size=1, k=200, alpha=0.2, beta=0.001, iterations=1000,
                 maximum_queries=10000, svm=False, momentum=0.0, tol=None, best_initial_target_sample=False, total_images=1000):
        self.model = model
        self.k = k
        self.alpha = alpha
        self.beta = beta
    ...
    def g_function_bin_search(self, model, image, theta, initial_lbd, true_labels, target_labels, tol=1e-3):
        ...
        if target_labels is None:
            lmdb_result, nquery = self.fine_grained_binary_search_local(image, true_labels, theta, initial_lbd, tol=tol)
        else:
            lmdb_result, nquery = self.fine_grained_binary_search_local_targeted(image, target_labels, theta, initial_lbd, tol=tol)
    ...
    def untargeted_attack(self, image_index, images, true_labels):
        ...
        grad_cosine_similarity = {'gamma': dict()}
        for i in range(self.iterations):
            ...
            true_grad = self.get_g_grad(self.model, images, xg, gg, true_label, None)
            true_grad = true_grad / torch.norm(true_grad, p=2, dim=(1, 2, 3), keepdim=True)
            gamma = F.cosine_similarity(true_grad.view(1, -1),
                                        (grad / torch.norm(grad, p=2, dim=(1, 2, 3), keepdim=True)).view(1, -1))
            grad_cosine_similarity['gamma'][query.item()] = gamma.detach().cpu().item()
    ...
    def attack_all_images(self, args, arch_name, result_dump_path):
        ...
        with open(result_dump_path, "w") as result_file_obj:
            ...
            with open('./logs/SignOPT_{}_{}_grad_cosine_similarity_result.json'.format(arch_name,
                    'targeted' if self.targeted else 'untargeted'), "w") as result_file:
