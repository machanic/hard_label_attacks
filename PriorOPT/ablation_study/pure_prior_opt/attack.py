...
from PriorOPT.ablation_study.pure_prior_opt.prior_opt_l2_norm_attack import PurePriorOptL2Norm
...
def get_exp_dir_name(dataset,  norm, targeted, target_type, args):
    ...
    if args.ablation_study:
        if args.sign:
            dirname = 'PurePriorSignOPT-{}-{}-{}/ablation_study'.format(dataset, norm, target_str)
        else:
            dirname = 'PurePriorOPT-{}-{}-{}/ablation_study'.format(dataset, norm, target_str)
        return dirname

    if args.best_initial_target_sample:
        if args.sign:
            if args.attack_defense:
                dirname = 'PurePriorSignOPT_best_start_initial_on_defensive_model-{}-{}-{}'.format(dataset, norm, target_str)
            else:
                dirname = 'PurePriorSignOPT_best_start_initial-{}-{}-{}'.format(dataset, norm, target_str)
        else:
            if args.attack_defense:
                dirname = 'PurePriorOPT_best_start_initial_on_defensive_model-{}-{}-{}'.format(dataset, norm, target_str)
            else:
                dirname = 'PurePriorOPT_best_start_initial-{}-{}-{}'.format(dataset, norm, target_str)
        return dirname
    if args.sign:
        if args.attack_defense:
            dirname = 'PurePriorSignOPT_on_defensive_model-{}-{}-{}'.format(dataset,  norm, target_str)
        else:
            dirname = 'PurePriorSignOPT-{}-{}-{}'.format(dataset, norm, target_str)
    else:
        if args.attack_defense:
            dirname = 'PurePriorOPT_on_defensive_model-{}-{}-{}'.format(dataset,  norm, target_str)
        else:
            dirname = 'PurePriorOPT-{}-{}-{}'.format(dataset, norm, target_str)
...
if __name__ == "__main__":
    ...
    for arch in archs:
        ...
        if args.tol is not None and args.tol != 0.0:
            tol = args.tol
        if args.norm == "l2":
            attacker = PurePriorOptL2Norm(model, surrogate_models, args.dataset, args.epsilon, args.targeted,
