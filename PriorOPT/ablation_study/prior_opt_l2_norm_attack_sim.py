...
class PriorOptL2Norm(object):
    def __init__(self, model, surrogate_models, dataset, epsilon, targeted, batch_size=1, k=100, alpha=0.2,
                 beta=0.001, iterations=1000, total_images=1000,maximum_queries=10000, sign=False, momentum=0.0, clip_grad_max_norm=1.0, tol=None,
    ...
    def prior_grad(self, images, theta, initial_lbd, true_label, target_label=None, grad_est_norm=None, sigma=0.001):
        ...
        for surrogate_model in self.surrogate_models:
            ...
            true_grad = self.get_g_grad(self.model, images, theta, initial_lbd, true_label, target_label)
            ...
            true_grad = true_grad / torch.norm(true_grad, p=2, dim=(1, 2, 3), keepdim=True)
            prior_grads.append(prior_grad)
            alpha = F.cosine_similarity(true_grad.view(1,-1), prior_grad.view(1,-1))
        ...
        gamma = F.cosine_similarity(true_grad.view(1,-1), (est_grad/torch.norm(est_grad, p=2, dim=(1, 2, 3), keepdim=True)).view(1,-1))
        return est_grad, query, grad_est_norm, alpha, gamma
    ...
    def prior_sign_grad(self, images, theta, initial_lbd, true_label, target_label=None, sigma=0.001):
        ...
        for surrogate_model in self.surrogate_models:
            ...
            prior_grads.append(prior_grad)
            alpha += F.cosine_similarity(true_grad.view(1, -1), prior_grad.view(1, -1))
        alpha /= len(self.surrogate_models)
        ...
        gamma = F.cosine_similarity(true_grad.view(1, -1),
                                    (sign_grad / torch.norm(sign_grad, p=2, dim=(1, 2, 3), keepdim=True)).view(1, -1))
        return sign_grad, query, alpha, gamma


    # def finetune_surrogate_model(self, surrogate_model, image, deque_theta_lmdb, true_labels, target_labels):
    #     surrogate_model.train()
    #     thetas = []
    #     lmdbs = []
    #     for theta, lmdb in list(deque_theta_lmdb):
    #         thetas.append(theta)
    #         lmdbs.append(lmdb)
    #     thetas = torch.cat(thetas, dim=0).to(image.device)
    #     lmdbs = torch.tensor(lmdbs).to(image.device)
    #     new_images = image + lmdbs.view(-1,1,1,1) * thetas
    #     true_labels_k = None
    #     if true_labels is not None:
    #         true_labels_k = true_labels.repeat(len(deque_theta_lmdb)).cuda()
    #     target_labels_k = None
    #     if target_labels is not None:
    #         target_labels_k = target_labels.repeat(len(deque_theta_lmdb)).cuda()
    #     optimizer = Adam(surrogate_model.parameters(), lr=0.001, betas=(0.9, 0.999))
    #     for iter in range(20):
    #         optimizer.zero_grad()
    #         loss = torch.square(self.cw_loss(surrogate_model(new_images.cuda()), true_labels_k, target_labels_k)).mean()
    #         loss.backward()
    #         optimizer.step()
    #     surrogate_model.eval()

    def PGD_attack(self, model, x_natural, y, step_size=0.003, epsilon=0.031, perturb_steps=10, distance='l_inf'):
    ...
    def untargeted_attack(self, image_index, images, true_labels):
        ...
        for i in range(num_directions):
            if self.PGD_init_theta:
                PGD_transfer_x_adv = self.PGD_attack(self.surrogate_models[0], images, true_label)
        ...
        grad_est_norm = None
        grad_cosine_similarity = {'alpha': dict(), 'gamma': dict()}
        for i in range(self.iterations):
            ## gradient estimation at x0 + theta (init)
            if self.sign:
                grad, grad_queries, a, g = self.prior_sign_grad(images, xg, gg, true_label, None, beta)
            else:
                grad, grad_queries, grad_est_norm, a, g = self.prior_grad(images, xg, gg, true_label, None, grad_est_norm, beta)
            grad_cosine_similarity['alpha'][query.item()] = a.detach().cpu().item()
            grad_cosine_similarity['gamma'][query.item()] = g.detach().cpu().item()
        ...
        # with open('swin.json', 'w') as file:
        #     json.dump(grad_cosine_similarity, file)
        # gg 是distortion
        distortion = torch.norm(gg * xg, p=2)
        assert distortion.item() - gg < 1e-4, "gg:{:.4f}  dist:{:.4f}".format(gg, distortion.item())
        return images + gg * xg, query,success_stop_queries, torch.tensor([gg]).float(), torch.tensor([gg]).float() <= self.epsilon, xg, grad_cosine_similarity
    ...
    def targeted_attack(self, image_index, images, target_labels, target_class_image):
        ...
        for i in range(self.iterations):
            if self.sign:
                grad, grad_queries, a, g = self.prior_sign_grad(images, xg, gg, None, target_label, beta)
            else:
                grad, grad_queries, grad_est_norm, a, g = self.prior_grad(images, xg, gg, None, target_label, grad_est_norm,  beta)
            grad_cosine_similarity['alpha'][query.item()] = a.detach().cpu().item()
            grad_cosine_similarity['gamma'][query.item()] = g.detach().cpu().item()
            grad = self.clip_grad_norm(grad, max_norm=self.clip_grad_max_norm)
            query += grad_queries
        ...
        return images + gg * xg, query, success_stop_queries, torch.tensor([gg]).float(), torch.tensor(
            [gg]).float() <= self.epsilon, xg, grad_cosine_similarity
    ...
    def attack_all_images(self, args, arch_name, result_dump_path):
        ...
        grad_cosine_similarity_results = dict()
        for batch_index, (images, true_labels) in enumerate(self.dataset_loader):
            ...
            if args.targeted:
                ...
                adv_images, query, success_query, distortion_with_max_queries, success_epsilon, theta_signopt, grad_cosine_similarity = self.targeted_attack(batch_index, images, target_labels, target_class_image)
            else:
                adv_images, query, success_query, distortion_with_max_queries, success_epsilon, theta_signopt, grad_cosine_similarity = self.untargeted_attack(batch_index,
                                                                                                                    images, true_labels)
            grad_cosine_similarity_results[int(batch_index + 1)] = grad_cosine_similarity
        ...
        with open('./logs/{}_{}_{}_grad_cosine_similarity_result.json'.format(
                'PriorSignOPT' if self.sign else 'PriorOPT', arch_name, 'targeted' if self.targeted else 'untargeted'), "w") as result_file:
            json.dump(grad_cosine_similarity_results, result_file, sort_keys=True)
