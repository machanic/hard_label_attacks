...
def prior_opt_expectation_gamma_square(alpha_list, q, d):
    s = len(alpha_list)
    expectation_gamma_lower_bound = math.sqrt(sum(alpha**2 for alpha in alpha_list) + (q-s)*(1-sum(alpha**2 for alpha in alpha_list))/math.pi *
                                              (np.exp(gammaln((d - s) / 2) - (gammaln((d -s+1)/ 2))))**2)
    expectation_gamma_upper_bound = math.sqrt(sum(alpha**2 for alpha in alpha_list) + 1.0/(d-s)*(2/math.pi*(q-s-1)+1)*(1-sum(alpha**2 for alpha in alpha_list)))
...
if __name__ == '__main__':
    ...
    for s in range(1,prior_number+1):
        ...
        prior_sign_opt_gamma_square_list.append(prior_sign_opt_expectation_gamma(random_alpha_list,q,d)[0])
        prior_opt_gamma_lower_bound_list.append(prior_opt_expectation_gamma_square(random_alpha_list,q,d)[0])
        prior_opt_gamma_upper_bound_list.append(prior_opt_expectation_gamma_square(random_alpha_list, q, d)[1])
        sorted_prior_sign_opt_gamma_square_list.append(prior_sign_opt_expectation_gamma(reversed_alpha_list,q,d)[0])
    ...
    plt.fill_between(x, y_prior_opt_sorted[:,0], y_prior_opt_sorted[:,1],color=colors[6],
                     label=r"Prior-OPT$_{\alpha_1\geq\alpha_2\geq\cdots\geq\alpha_s}$")
    plt.fill_between(x, y_prior_opt_lower_bound, y_prior_opt_upper_bound, color=colors[2],
                     label=r"Prior-OPT$_{\mathregular{Shuffle}\{\alpha_1,\alpha_2,\cdots,\alpha_s\}}$")
