...
if __name__ == '__main__':
    ...
    for q in q_list:
        ...
        prior_sign_opt_gamma_square_2 = prior_sign_opt_expectation_gamma(alpha_2,q,d)[0]
        ...
        prior_sign_opt_gamma_square_5 = prior_sign_opt_expectation_gamma(alpha_5, q, d)[0]
    ...
    plt.fill_between(x, y_prior_opt_2[:, 0], y_prior_opt_2[:, 1], label=r"Prior-OPT$_{\alpha=0.2}$", color=colors[2])
    ...
    plt.fill_between(x, y_prior_opt_5[:, 0], y_prior_opt_5[:, 1], label=r"Prior-OPT$_{\alpha=0.1}$", color=colors[4])
    ...
    # plt.xticks(xtick, xtick, fontsize=18)
    # plt.yticks(np.linspace(0, max_y, 11),fontsize=18)
    ...
    plt.ylabel(r"$\mathbb{E}[\gamma]$", fontsize=25)
    plt.legend(loc='lower right', prop={'size': 20}, framealpha=0.5, fancybox=True, frameon=True)
    plt.savefig("./query_vs_gamma_d={}.pdf".format(d), dpi=200)
    plt.close()
