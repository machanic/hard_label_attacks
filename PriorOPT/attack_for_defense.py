...
def get_parse_args():
    ...
    parser.add_argument('--norm', type=str, required=True, choices=["l2", "linf"],
                        help='Which lp constraint to run bandits [linf|l2]')
    ...
    parser.add_argument('--sign', action='store_true',
                        help="whether to use the sign-based prior-guided gradient estimation")
    parser.add_argument('--clip-grad-max-norm', type=float)
    parser.add_argument('--tol', type=float, )
    parser.add_argument('--prior-grad-bs-tol', type=float, default=0.01,
                        help="the binary search's stopping threshold for estimating gradient")
    ...
    for arch in archs:
        ...
        if args.norm == "l2":
            attacker = PriorOptL2Norm(model, surrogate_models, args.dataset, args.epsilon, args.targeted,
                                      ...
                                      maximum_queries=args.max_queries, sign=args.sign,
                                      clip_grad_max_norm=args.clip_grad_max_norm,
                                      tol=tol, prior_grad_binary_search_tol=args.prior_grad_bs_tol,
                                      best_initial_target_sample=args.best_initial_target_sample, PGD_init_theta=args.PGD_init_theta)
            ...
            attacker = PriorOptLinfNorm(model, surrogate_models, args.dataset, args.epsilon, args.targeted,
                                        ...
                                        prior_grad_binary_search_tol=args.prior_grad_bs_tol,
