...
from PBOPT.bo_linf import BayesOpt
...
class Objective(object):
    ...
    def __call__(self, pert, ldb):
        ...
        for i in range(len(image_pert)):
            theta = image_pert[i]/torch.norm(image_pert[i], p=2)  #FIXME
            _res, query = self.fine_grained_binary_search(self.data, theta, ldb)
            distorition = torch.norm(torch.clamp(self.data + _res * theta, min=0., max=1.) - self.data, p=float('inf'))
...
class DistortionWithCWGrad(torch.autograd.Function):
    ...
    def forward(ctx, x0, theta, ldb, target, is_targeted, fine_grained_search_func):
        # theta shape = (B,C,H,W)
        norms = theta.norm(p=float("inf"), dim=(1, 2, 3), keepdim=True)
        norms = norms.clamp_min(1e-12)
        theta = theta / norms
        with torch.no_grad():
            linf_distance = fine_grained_search_func(x0, theta, ldb)  # shape = (B,)
        ctx.save_for_backward(x0, theta, linf_distance)
        ctx.is_targeted = is_targeted
        ctx.target = target
        return linf_distance  # (B,)；B=1 时就是形状 (1,)

    @staticmethod
    def backward(ctx, grad_output):
        x0, theta, linf_distance = ctx.saved_tensors
        B = theta.size(0)
        linf_distance = linf_distance.view(B, 1, 1, 1)
        finite_mask = torch.isfinite(linf_distance.view(-1))
        ...
        with torch.enable_grad():
            ...
            lambda_sel = linf_distance.index_select(0, idx).detach().requires_grad_(True)
            x_adv_sel = torch.clamp(x0 + lambda_sel * theta_sel/torch.norm(theta_sel,p=float('inf'),dim=tuple(range(1, theta_sel.ndim)),keepdim=True), 0, 1.0)  # 有可能传入无穷大的l2_distance
...
class SurrogateObjective(Objective):
    ...
    def __call__(self, theta, ldb, use_parallel_search=True):
        ...
        if self.original_size != self.reduce_size:
            if self.mode == 'bilinear':
                theta = torch.nn.functional.interpolate(theta, [self.original_size, self.original_size],
                ...
                raise
        linf_distance = DistortionWithCWGrad.apply(
            self.data,
            theta,
            ldb,  # 当前的扰动幅度
        ...
        distortion_full = torch.full((B,), float("inf"), device=linf_distance.device, dtype=self.data.dtype)
        if idx.numel() > 0:
            lambda_sel = linf_distance.index_select(0, idx).view(-1, 1, 1, 1)  # (M,1,1,1)
            theta_sel = theta.index_select(0, idx)  # (M,C,H,W)
            theta_sel = theta_sel / torch.norm(theta_sel, dim=tuple(range(1, theta_sel.ndim)), p=float('inf'), keepdim=True)
            adv_x_sel = torch.clamp(self.data + lambda_sel * theta_sel, 0., 1.)
            distortion_sel = torch.norm(adv_x_sel - self.data, p=float('inf'), dim=tuple(range(1, adv_x_sel.ndim)))  # (M,)
    ...
    def attack_all_images(self, args, tmp_result_path, result_json_path):
        ...
        if osp.exists(tmp_result_path):
            with open(tmp_result_path, "r") as file_obj:
                ...
                for key in ['query_all', 'correct_all', 'distortion']:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--norm', type=str, default='linf', choices=["linf"], required=True, help='Which lp constraint to run bandits [linf|l2]')
    ...
    log.info(f"Using PyTorch with version :{torch.__version__}, CUDA version: {torch.version.cuda}.")  # PyTorch 版本（如 2.3.0+cu121）
    ...
    if args.attack_defense:
        if args.surrogate_arch is not None:
            if args.dataset == "ImageNet":
                defense_surrogate_arch_str = "{surrogate_arch}({surrogate_defense_model}_{surrogate_defense_norm}_{surrogate_defense_eps})".format(surrogate_arch=args.surrogate_arch,
                                 surrogate_defense_model="AT" if args.surrogate_defense_model.startswith("adv_train") else args.surrogate_defense_model, surrogate_defense_norm=args.surrogate_defense_norm, surrogate_defense_eps=args.surrogate_defense_eps)
            else:
                defense_surrogate_arch_str = "{surrogate_arch}({surrogate_defense_model})".format(surrogate_arch=args.surrogate_arch,
            ...
            if args.dataset == "ImageNet":
                log_file_path = osp.join(args.exp_dir,
                                         "run_{arch}({arch_defense_model}_{arch_defense_norm}_{arch_defense_eps})_surrogate_{surrogate_arch}.log".format(
                                             ...
                                             arch_defense_eps=args.defense_eps, surrogate_arch=defense_surrogate_arch_str))
                save_result_path = osp.join(args.exp_dir, "/{arch}({arch_defense_model}_{arch_defense_norm}_{arch_defense_eps})_surrogate_{surrogate_arch}.json".format(
                    arch=args.arch,
                ...
                log_file_path = osp.join(args.exp_dir, "run_{arch}({arch_defense_model})_surrogate_{surrogate_arch}.log".format(
                                             arch=args.arch, arch_defense_model="AT" if args.defense_model.startswith(
                                                 "adv_train") else args.defense_model, surrogate_arch=defense_surrogate_arch_str))
                save_result_path = osp.join(args.exp_dir, "/{arch}({arch_defense_model})_surrogate_{surrogate_arch}.json".format(
                    arch=args.arch,
                    arch_defense_model="AT" if args.defense_model.startswith("adv_train") else args.defense_model,
                    surrogate_arch=defense_surrogate_arch_str))
        else:
            if args.dataset == "ImageNet":
                log_file_path = osp.join(args.exp_dir, "run_{arch}({arch_defense_model}_{arch_defense_norm}_{arch_defense_eps}).log".format(
                                             arch=args.arch, arch_defense_model="AT" if args.defense_model.startswith(
                                                 "adv_train") else args.defense_model,
                                             arch_defense_norm=args.defense_norm,
                                             arch_defense_eps=args.defense_eps))
                save_result_path = osp.join(args.exp_dir, "/{arch}({arch_defense_model}_{arch_defense_norm}_{arch_defense_eps}).json".format(
                    arch=args.arch,
                    arch_defense_model="AT" if args.defense_model.startswith("adv_train") else args.defense_model,
                    arch_defense_norm=args.defense_norm,
                    arch_defense_eps=args.defense_eps))
    ...
    if args.surrogate_arch is not None:
        if args.attack_defense:
            ...
            surrogate_model = StandardModel(args.dataset, args.surrogate_arch, no_grad=False)
