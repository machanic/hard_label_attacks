...
def get_orthogonal_1d_in_subspace(side_length, x_o2x_adv: torch.Tensor, n, ratio_size_mask=0.3, if_left=1) -> torch.Tensor:
    random.seed(time.time())
    zero_mask = torch.zeros(size=[side_length, side_length]).cuda()
    size_mask = int(side_length * ratio_size_mask)
    if if_left:
    ...
    mask1 = mask1.reshape(-1, side_length, side_length)

    select = np.random.choice(len(x), size=n, replace=False)
    mask2 = torch.zeros_like(zero_mask)
    mask2[x[select], y[select]] = 1
    mask2 = mask2.reshape(-1, side_length, side_length)

    select = np.random.choice(len(x), size=n, replace=False)
    mask3 = torch.zeros_like(zero_mask)
    mask3[x[select], y[select]] = 1
    mask3 = mask3.reshape(-1, side_length, side_length)

    mask = torch.cat([mask1, mask2, mask3], dim=0).expand([1, 3, side_length, side_length])
    mask *= torch.randn_like(mask).cuda()
...
#         direction, mask = get_orthogonal_1d_in_subspace(args.side_length, x_o2x_adv, dim_num, args.ratio_mask, args.dim_num)
...
#         dist1 = torch.norm((x_hat - x_o).contiguous().view(1, -1), 2, 1)
#         print(queries, dist, dist1)
