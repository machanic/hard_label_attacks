...
class SQBA_Attack(object):
    def __init__(self, model, dataset, clip_min, clip_max, height, width, channels, norm, epsilon, load_random_class_image,
                 surrogate_model, total_images, dgm_cfg=None, sqba_cfg=None,
    ...
    def check_adversary(self, x):
        _, pred = self.model(torch.clamp(x.reshape(self.shape), min=self.clip_min, max=self.clip_max)).data.max(1)
        self.query += 1
        flag = False
        if self.targeted:
            if pred == self.label:
    ...
    def black_update(self, x_adv, delta, num_eval):
        ...
        if delta == 0 or num_eval == 0:
            log.info("delta {:.3f}, num_eval {}".format(delta, num_eval))
        ...
        if torch.isnan(g).any():
            log.info("result Nan - {:.5f}".format(torch.norm(grad)))
    ...
    def perturb(self, x_best):
        ...
        while True:
            ...
            success = self.check_adversary(x_c)
            count_halving += 1
            if success:
                break
            if count_halving > max_halving or epsilon < 1e-10:
                log.info(f"Failed! The program cannot find success within {count_halving} halvings.")
                break
    ...
    def fine_grained_binary_search(self,  x0, y0, theta, initial_lbd, current_best):
        ...
        while (lbd_hi - lbd_lo) > 1e-3:  # was 1e-5
            ...
            if self.check_adversary(torch.clamp(x0 + lbd_mid * theta, self.clip_min, self.clip_max)):
                lbd_hi = lbd_mid
            else:
                lbd_lo = lbd_mid
            if count >= 200:
    ...
    def initialize_adv_with_PGD(self, images, true_labels):
        ...
        PGD_transfer_x_adv = self.PGD_attack(self.surrogate_model, images, true_labels)
        num_evals += 1
        # if self.model(PGD_transfer_x_adv).max(1)[1].item() != true_label:
        if self.check_adversary(PGD_transfer_x_adv):
        ...
        while True:
            ...
            if self.check_adversary(images + theta):
            ...
            if torch.linalg.norm(theta.view(-1)).item() >= 10 * torch.linalg.norm(images.view(-1)).item():
                ...
                log.info(
                    "PGD θ initialization failed! Original image prediction is {}, image+θ prediction is {}, true label is {}, θ norm is {:.4f}, image norm is {:.4f}.".format(
                        self.model(images).max(1)[1].item(),
                        self.model(torch.clamp(images + theta, self.clip_min, self.clip_max)).max(1)[1].item(),
                        true_label,
                ...
                while not self.check_adversary(images + theta):
                    num_evals += 1
                    temp_evals += 1
                    theta *= 2
                    if temp_evals >= 100:
                ...
                num_evals += count
                break
        return torch.clamp(images + lbd * theta / torch.norm(theta.view(-1), p=2).item(), self.clip_min, self.clip_max), num_evals

        # return images + lbd * theta / torch.norm(theta.view(-1), p=2).item(), num_evals
    ...
    def attack(self, batch_index, images, target_images, true_labels, target_labels):
        ...
        if self.sqba_cfg.on:
            ...
            while self.query < self.maximum_queries:
                if sm == 0:
                    x_adv, num_eval = self.initialize_adv_with_PGD(x, self.label)
                    x_adv = x_adv.flatten()
...
def get_exp_dir_name(dataset,  norm, targeted, target_type, args):
    ...
    if args.attack_defense:
        dirname = 'SQBA(PGD)_on_defensive_model-{}-{}-{}'.format(dataset, norm, target_str)
    else:
        dirname = 'SQBA(PGD)-{}-{}-{}'.format(dataset, norm, target_str)
    return dirname
...
if __name__ == "__main__":
    ...
    parser.add_argument("--surrogate-arch", required=True, type=str, help="the architecture of a surrogate model")
    ...
    assert args.targeted == False, "targeted attack is not supported yet"
            ...
            if args.dataset == "ImageNet":
                log_file_path = osp.join(args.exp_dir,
                                         "run_{arch}({arch_defense_model}_{arch_defense_norm}_{arch_defense_eps})_surrogate_{surrogate_arch}.log".format(
                                             arch=args.arch, arch_defense_model="AT" if args.defense_model.starswith(
        ...
        if args.attack_defense:
            if args.dataset == "ImageNet":
                save_result_path = args.exp_dir + "/{arch}({arch_defense_model}_{arch_defense_norm}_{arch_defense_eps})_surrogate_{surrogate_arch}.json".format(
                    arch=args.arch,
                    arch_defense_model="AT" if args.defense_model.starswith("adv_train") else args.defense_model,
