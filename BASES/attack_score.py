...
class BASES(object):
    def __init__(self, model, surrogate_models, dataset, clip_min, clip_max, height, width, channels, norm, epsilon, load_random_class_image,
        ...
        self.n_iters = 10
        self.alpha = 3/255 * self.epsilon / self.n_iters
        self.lr_w = 5e-3
        self.fuse = 'loss'
        self.loss_name = 'cw'
    ...
    def attack(self, batch_index, images, target_images, true_labels, target_labels):
        ...
        label_idx, loss, _ = get_label_loss(adv_np, self.model, tgt_label, self.loss_name, targeted=args.targeted)
        n_query = 1
        w_list = []
        ...
        # pretend
        # if not args.untargeted and label_idx != true_labels:
        #     success_idx_list_pretend.add(im_idx)
        #     query_list_pretend.append(n_query)

        if (args.targeted and label_idx == tgt_label) or (not args.targeted and label_idx != tgt_label):
            ...
            while n_query < args.iterw:
                ...
                label_plus, loss_plus, _ = get_label_loss(adv_np_plus, self.model, tgt_label, self.loss_name,
                ...
                adv_np_minus, losses_minus = get_adv_np(images, tgt_label, w_np_temp_minus, self.surrogate_models,
                                                        ...
                                                        self.alpha, fuse=self.fuse, untargeted=not args.targeted,
                                                        loss_name=self.loss_name, adv_init=adv_np)
                label_minus, loss_minus, _ = get_label_loss(adv_np_minus, self.model, tgt_label, self.loss_name,
                                                            targeted=args.targeted)
                n_query += 1
                ...
                # stop if successful
                if (args.targeted) * (tgt_label == label_minus) or not args.targeted * (
                        tgt_label != label_minus):
                    print('success')
                    # success_idx_list.add(im_idx)
                    # query_list.append(n_query)
                    loss = loss_minus
...
if __name__ == "__main__":
    ...
    for arch in archs:
        ...
        model.cuda()
        model.eval()
        attacker = BASES(model, surrogate_models, args.dataset, 0, 1.0, model.input_size[-2], model.input_size[-1], IN_CHANNELS[args.dataset],
