    ...
    def get_logits(self, pert):
        ...
        if self.original_size != self.reduce_size:
            if self.mode == 'bilinear':
                ...
                raise
        image = torch.clamp(self.data + torch.clamp(image_pert, -self.epsilon, self.epsilon), 0, 1)
    ...
    def get_logits(self, pert, eps):
        ...
        if self.original_size != self.reduce_size:
            if self.mode == 'bilinear':
                ...
                raise
        image = torch.clamp(self.data + image_pert * eps, 0, 1)
        res = self.model(image)
        return res.detach() if self.nograd else res

    def get_loss(self, pert, eps):
        res = cw_loss(self.get_logits(pert, eps), self.target, is_targeted=self.is_targeted)
        return res.detach() if self.nograd else res

        # image_pert = pert.reshape([-1, 3, self.reduce_size, self.reduce_size])
        # image = torch.clamp(self.data + image_pert * eps, 0, 1)
        # pred = (self.predict(pert, eps) != self.target).float()
        # res = -torch.norm((image - self.data).view(1, -1), p=np.inf) + self.lamda * (pred - 1)
        # return res

    def __call__(self, pert, eps):
        return self.get_loss(pert, eps)

    def get_grad(self, pert, eps):
        if self.nograd:
            raise NotImplementedError("Cannot get grad if self.nograd is True")
        x = pert.detach()
        x.requires_grad_()
        # obj = self.get_loss(x, eps)
        obj = cw_loss(self.get_logits(pert, eps), self.target, is_targeted=self.is_targeted)
        obj.backward()
        return x.grad.detach()

    def predict(self, pert, eps):
        res = self.get_logits(pert, eps).max(dim=1)[1]
        return res.detach() if self.nograd else res

    # def check_success(self, pert, eps):
    #     pred = self.predict(pert, eps)
    #     if not self.is_targeted:
    #         assert all(pred != self.target)
    #     else:
    #         assert all(pred == self.target)
        ...
        for batch_idx, (images, true_labels) in enumerate(self.dataset_loader):
            ...
            if not args.targeted:
                ...
                obj = Objective(target_model, data, target, reduce_size=reduce_size,
                                epsilon=0.1, is_targeted=args.targeted, mode=args.dr, nograd=True)
                prior_obj = Surrogate_Objective(self.surrogate_model, data, target, reduce_size=reduce_size,
