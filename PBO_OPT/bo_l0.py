...
class Func:
    ...
    def __call__(self, tensor, save_query=True):
        y, q = self.func(tensor)#.detach()
        if save_query:
            if self.prior_func:
...
class BayesOpt:
    ...
    def find_next(self, best, iter):
        ...
        # print(objective)
        # print([-torch.log10(-self.f(x0, False)) for x0 in x])
        ...
        with gpytorch.settings.fast_computations(covar_root_decomposition=False, log_prob=False, solves=False):
            with gpytorch.settings.cholesky_max_tries(6):
                for i in range(n_iter):
                    ...
                    x = self.find_next(self.f.get_opt(), iter).detach()
                    self.update_model(x)
                    opt = self.f.get_opt()
