...
from config import IN_CHANNELS, CLASS_NUM, MODELS_TEST_STANDARD, PROJECT_PATH
...
class Objective:
    ...
    def fine_grained_binary_search(self, x0, theta, initial_lbd=4.0):
        lbd = initial_lbd
        query = 1
        if self.model(torch.clamp(x0 + lbd * theta, min=0., max=1.)).max(1)[1] == self.target:
            lbd_lo = lbd
            lbd_hi = lbd * 1.1
            while self.model(torch.clamp(x0 + lbd_hi * theta, min=0., max=1.)).max(1)[1] == self.target:
                lbd_hi = lbd_hi * 1.1
                query += 1
        else:
            # lbd_hi = lbd
            ...
            lbd_hi = lbd
            lbd_lo = lbd * 0.9
            while self.model(torch.clamp(x0 + lbd_lo * theta, min=0., max=1.)).max(1)[1] != self.target:
                lbd_lo = lbd_lo * 0.9
                query += 1

        tot_count = 0
        old_lbd_mid = lbd_hi
        while (lbd_hi - lbd_lo) > 0.01 * initial_lbd:
            tot_count += 1
            lbd_mid = (lbd_lo + lbd_hi) / 2.0
            query += 1
            if self.model(torch.clamp(x0 + lbd_mid * theta, min=0., max=1.)).max(1)[1] != self.target:
                lbd_hi = lbd_mid
            else:
                lbd_lo = lbd_mid
    ...
    def __call__(self, pert, ldb=4.0):
    ...
    def attack_all_images(self, args, arch_name, target_model, result_dump_path):
        ...
        for batch_idx, (images, true_labels) in enumerate(self.dataset_loader):
            ...
            if not args.targeted:
                ...
                prior_obj = Surrogate_Objective(self.surrogate_model, data, target, original_size=target_model.input_size[-1], reduce_size=reduce_size,
...
if __name__ == "__main__":
    ...
    parser.add_argument('--json-config', type=str, default='TangentAttack-main/configures/QEDBA.json',
