...
class Func:
    ...
    def __call__(self, tensor, ldb=4., save_query=True):
...
class BayesOpt:
    ...
    def gen_rand_init(self, n_rand_init=10):
        ...
        # x_init = torch.tensor(sampler.random(n=n_rand_init), dtype=torch.float32)
        # sobol_engine = torch.quasirandom.SobolEngine(dimension=self.dim, scramble=True)
        # x_init = sobol_engine.draw(n_rand_init).cuda()
    ...
    def find_next(self, best, iter):
        ...
        x.requires_grad_()
        # lower, upper = x.detach() - 0.03, x.detach() + 0.03
        ...
        def gen_objective(x):
            if not self.prior_f or self.scale != 'fixed_only':
                ...
                '''EI'''
                # std = torch.clamp(torch.sqrt(pred.variance), min=1e-6)
                # improvement = pred.mean - best
                # Z = improvement / std
                # normal = Normal(0, 1)
                # ans = improvement * normal.cdf(Z) + std * normal.log_prob(Z).exp()
            else:
                ans = 0.
            if self.prior_f:
        ...
        for i in range(n_iter_opt_acq):
            ...
            # _x = torch.clamp(x + lr * x.grad, min=self.lower, max=self.upper)
    ...
    def run(self, n_iter=10000, freq_fit_hyper=10):
        if self.scale != 'fixed_only':
        ...
        with gpytorch.settings.fast_computations(covar_root_decomposition=False, log_prob=False, solves=False):
            with gpytorch.settings.cholesky_max_tries(6):
                for i in range(n_iter):
                    ...
                    msg = f'{i} q={self.f.call_count} best_f={opt} f={newest}'
                    if self.prior_f:
