...
class AHA(object):
    ...
    def attack(self, batch_index, images, target_images, true_labels, target_labels, image_size, correct):
        ...
        if target_images is not None:
            target_images = target_images.squeeze().cuda()
        ...
        def test(adv):
            ...
            if target_labels is None:
                return (pred.max(1)[1] != true_labels).float()
            else:
                return (pred.max(1)[1] == target_labels).float()
        ...
        while torch.min(query) <= self.maximum_queries:
            ...
            eta = self.sample_gaussian_torch(bias.size()).cuda()
            z = to_orig_direction.abs()
            z = torch.nn.functional.interpolate(z, (DIM, DIM)).cuda()
            eta = eta * z + bias
