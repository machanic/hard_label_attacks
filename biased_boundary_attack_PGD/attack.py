...
from biased_boundary_attack_PGD.bba_utils.sampling.sampling_provider import SamplingProvider
from biased_boundary_attack_PGD.bba_utils.util import sample_hypersphere
...
class BiasedBoundaryAttack(object):
    ...
    def __init__(self, model, substitute_model,  dataset, norm, epsilon,
                 batch_size, targeted, clip_min, clip_max, maximum_queries, load_random_class_image, total_image,
                 USE_PERLIN_BIAS, USE_MASK_BIAS):
        """
        ...
        img_shape = self.model.input_size  # 3,224,224
        ...
        self.total_images = total_image#len(self.dataset_loader.dataset)
    ...
    def find_closest_img(self, X_orig, X_targets, label, is_targeted):
        ...
        """
        if X_orig.dim() == 4:
            X_orig = torch.squeeze(X_orig, dim=0)
    ...
    def attack(self, image_index, image,image_start,label, n_max_per_batch=50,
        ...
        try:
            ...
            while n_calls_left_fn(self.query) > 0:
    ...
    def _eval_sample(self, x, x_orig=None):
        ...
        if x_orig is None:
            ...
            dist = torch.norm((x - x_orig).view(1, -1), p=self.ord, dim=1)
            return label, dist
    ...
    def initialize_adv_with_noise(self, sample, true_labels):
        """
        ...
        while True:
            ...
            success = self.decision_function(random_noise, true_labels, None)
            num_eval += 1
            if success:
                break
            if num_eval > 1000:
        ...
        while high - low > 0.001:
            ...
            success = self.decision_function(blended, true_labels, None)
            num_eval += 1
            if success:
                high = mid
            else:
    ...
    def PGD_attack(self, model, x_natural, y, step_size=0.003, start_random_coeff=0.001, epsilon=0.031, perturb_steps=10, distance='linf'):
        ...
        for _ in range(perturb_steps):
            ...
            if distance == 'linf':
                ...
                x_adv = x_adv.detach() + step_size * grad.detach() /self.normalize(grad.detach())
    ...
    def fine_grained_binary_search(self,  x0, y0, theta, initial_lbd, current_best):
        ...
        if initial_lbd > current_best:
            nquery += 1
            if self.model(torch.clamp(x0 + current_best * theta, 0.0, 1.0)).max(1)[1].item() == y0:
        ...
        while (lbd_hi - lbd_lo) > 1e-3:  # was 1e-5
            ...
            nquery += 1
            count += 1
            if self.model(torch.clamp(x0 + lbd_mid * theta, 0.0, 1.0)).max(1)[1].item() != y0:
                lbd_hi = lbd_mid
            else:
                lbd_lo = lbd_mid
            if count >= 200:
    ...
    def initialize_adv_with_PGD(self, images, true_labels):
        num_evals = 0
        temp_evals = 0
        true_label = true_labels[0].item()
        PGD_transfer_x_adv = self.PGD_attack(self.substitute_model, images, true_labels)
        num_evals += 1
        if self.model(PGD_transfer_x_adv).max(1)[1].item() != true_label:
            return PGD_transfer_x_adv, num_evals
        theta = PGD_transfer_x_adv - images
        while True:
            num_evals += 1
            temp_evals += 1
            if self.model(torch.clamp(images + theta, 0.0, 1.0)).max(1)[1].item() != true_label:
                initial_lbd = torch.norm(theta.view(-1), p=2).item()
                theta /= initial_lbd
                lbd, count = self.fine_grained_binary_search(images, true_label, theta, initial_lbd, float('inf'))
                num_evals += count
                break
            else:
                theta *= 2
            if torch.linalg.norm(theta.view(-1)).item() >= 10 * torch.linalg.norm(images.view(-1)).item():
                num_evals -= temp_evals
                log.info(
                    "PGD θ initialization failed! Original image prediction is {}, image+θ prediction is {}, true label is {}, θ norm is {:.4f}, image norm is {:.4f}.".format(
                        self.model(torch.clamp(images, 0.0, 1.0)).max(1)[1].item(),
                        self.model(torch.clamp(images + theta, 0.0, 1.0)).max(1)[1].item(),
                        true_label,
                        torch.linalg.norm(theta.view(-1)).item(), torch.linalg.norm(images.view(-1)).item()))
                PGD_transfer_x_adv = self.PGD_attack(self.substitute_model, images, true_labels,
                                                     epsilon=self.epsilon, perturb_steps=10, distance="l2")
                theta = PGD_transfer_x_adv - images
                temp_evals = 0
                L2_PGD_fail = False
                while self.model(torch.clamp(images + theta, 0.0, 1.0)).max(1)[1].item() == true_label:
                    num_evals += 1
                    temp_evals += 1
                    theta *= 2
                    if temp_evals >= 100:
                ...
                num_evals += count
                break
        return torch.clamp(images + lbd * theta / torch.norm(theta.view(-1), p=2).item(), min=self.clip_min, max=self.clip_max), num_evals
    ...
    def attack_all_images(self, args, arch_name, result_dump_path):
        ...
        for batch_index, (images, true_labels) in enumerate(self.dataset_loader):
            ...
            if args.targeted:
                ...
                target_images = select_random_image_of_target_class(self.dataset_name, target_labels, self.model, args.load_random_class_image)
                ...
                random_noise = torch.from_numpy(
                    np.random.uniform(self.clip_min, self.clip_max, size=(100,)+self.shape)).float().to(self.device)
                # We just test the abnormal condition, so we don't count the query at this time.
                logits_random_noise = self.model(random_noise)
                pred_labels_random_noise = torch.unique(logits_random_noise.max(1)[1])
                if pred_labels_random_noise.size()[0] == 1 and pred_labels_random_noise.item() == label:
                    log.info("{}-th image: all random noises are predicted as the same true label: {}, skip!".format(batch_index + 1, label))
                    continue
                target_images, num_eval = self.initialize_adv_with_PGD(images, true_labels)
                assert target_images is not None, log.error("Error! Target image is None at the {}-th image.".format(batch_index+1))
                target_images = target_images.cuda()
                self.query += num_eval
...
def get_exp_dir_name(dataset,  norm, targeted, target_type, args):
    ...
    if args.attack_defense:
        dirname = 'BBA(PGD)_on_defensive_model-{}-{}-{}'.format(dataset,  norm, target_str)
    else:
        dirname = 'BBA(PGD)-{}-{}-{}'.format(dataset, norm, target_str)
    return dirname
...
if __name__ == "__main__":
    ...
    parser.add_argument('--use-perlin-bias', action="store_true")
    parser.add_argument('--use-mask-bias', action="store_true")
    parser.add_argument('--use-surrogate-bias', action="store_true")
    ...
    for arch in archs:
        ...
        attacker = BiasedBoundaryAttack(model, substitude_model, args.dataset, args.norm, args.epsilon, args.batch_size,
                                        args.targeted, 0.0, 1.0, args.max_queries, args.load_random_class_image, args.total_images,
                                        args.use_perlin_bias, args.use_mask_bias)
